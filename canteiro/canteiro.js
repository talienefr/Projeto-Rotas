// canteiro.js - Cadastro com fallback manual de coordenadas

document.getElementById('cep').addEventListener('blur', buscarEnderecoViaCEP);

document.getElementById('form-canteiro').addEventListener('submit', async function (e) {
  e.preventDefault();
  ...
  if (!nome || !rua || !numero || !bairro || !cidade || !estado || !cep || !statusSelecionado) {
  ...
  try {
    ...
    if (resposta.ok) {
      ...
      mensagem.textContent = 'Canteiro cadastrado com sucesso!';
      document.getElementById('form-canteiro').reset();
      document.getElementById('coordenadas-manual').style.display = 'none';
    } else {
      mensagem.style.color = 'red';
      mensagem.textContent = resultado.error || 'Erro ao cadastrar canteiro.';
    }
  } catch (erro) {
